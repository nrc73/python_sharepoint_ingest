"""Plain-text email body builders for ingestion notifications.

These functions are pure string constructors — they have no dependency on
:class:`~sharepoint_ingest.notifications.EmailNotifier` or any SMTP transport.
Keeping them separate makes them easy to unit-test and reuse outside the
notification layer (e.g. in diagnostic tools or custom alerting pipelines).

They are also re-exported from :mod:`sharepoint_ingest.notifications` so
existing callers that import from there continue to work unchanged.
"""
