...
class IngestionEngine:
    ...
    def _process_parquet_file_in_chunks(
        ...
        2. Open ParquetFile via SharePointRangeReader (footer only, 2–3 range requests).
        3. **Single iteration** — each row group fetched once:
           a. Transform chunk (column mapping, metadata, normalisation).
           b. Validate chunk in-flight; accumulate issues.
           c. Append chunk to a transient SQL staging table ``_tmp_<table>_<uid>``.
        4. After the full pass:
           a. Publish validation results; abort on blocking errors (drop temp).
           b. APPEND → SQL-side PK dup check on temp table.
           c. Atomic swap temp → destination (TRUNCATE+INSERT or INSERT).

        This halves HTTP range requests and per-row CPU work compared to the
        previous two-pass approach.
        """
        # ── 1. File metadata ──────────────────────────────────────────────────
